...
function euclidianDist(p1,p2, sqDist=false){
    const diff = vectorAdd([p1,vectorScale(p2,-1)])
    let sqDistLen = 0
    for(let i=0;i<diff.length;i++){
        sqDistLen+= diff[i]*diff[i]
    } 

    if(sqDist){return sqDistLen}
    else{return sqrt(sqDistLen)}
}